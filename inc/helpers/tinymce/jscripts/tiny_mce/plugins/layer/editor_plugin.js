...
var TinyMCE_LayerPlugin = {getInfo: function () {
    ...
    if (tinyMCE.isMSIE && !tinyMCE.isOpera)inst.getDoc().execCommand('2D-Position')
    ...
    for (i = 0; i < nl.length; i++) {
        if (new RegExp('absolute|relative|static', 'gi').test(nl[i].style.position)) {
            if (state)tinyMCE.addCSSClass(nl[i], 'mceVisualAid'); else tinyMCE.removeCSSClass(nl[i], 'mceVisualAid')
        }
    }
    ...
    if (p)tinyMCE.switchClass(editor_id + '_absolute', 'mceButtonNormal');
    ...
    if (!h)h = tinyMCE.getLang('lang_layer_content');
    ne.innerHTML = h;
    d.body.appendChild(ne)
}, _toggleAbsolute: function () {
    var inst = tinyMCE.selectedInstance;
    var le = this._getParentLayer(inst.getFocusElement());
    if (le == null)le = tinyMCE.getParentElement(inst.getFocusElement(), 'div,p,img');
    if (le) {
