...
var TinyMCE_FullScreenPlugin = {getInfo: function () {
    ...
    if (!tinyMCE.settings['fullscreen_skip_plugin_css'])tinyMCE.importCSS(inst.getDoc(), tinyMCE.baseURL + "/plugins/fullscreen/css/content.css")
    ...
    if (!ds.enabled) {
        ...
        tinyMCE.getParentNode(tableElm.parentNode, function (n) {
            ...
            if (n.nodeType == 1 && st) {
                ...
                ds.parents.push({el: n, position: st.position, left: st.left, top: st.top, right: st.right, bottom: st.bottom, width: st.width, height: st.height, margin: st.margin, padding: st.padding, border: st.border});
                st.position = 'static';
        ...
        if (ds.oldWidth && ds.oldWidth.indexOf)ds.oldTWidth = ds.oldWidth.indexOf('%') != -1 ? ds.oldWidth : ds.oldTWidth;
        ...
        if (re && tinyMCE.getParam("theme_advanced_resizing", false))re.style.display = 'block';
