...
var TinyMCE_MediaPlugin = {getInfo: function () {
    ...
    if (!tinyMCE.settings['media_skip_plugin_css'])tinyMCE.importCSS(inst.getDoc(), tinyMCE.baseURL + "/plugins/media/css/content.css")
    ...
    switch (type) {
        ...
        case"get_from_editor":
            ...
            while ((startPos = content.indexOf('<img', startPos + 1)) != -1) {
                ...
                if (!/mceItem(Flash|ShockWave|WindowsMedia|QuickTime|RealMedia)/.test(attribs['class']))continue;
                endPos += 2;
                at = attribs['title'];
                if (at) {
                    at = at.replace(/&#39;/g, "'");
                ...
                if (!tinyMCE.getParam('media_use_script', false)) {
                    ...
                    if (!tinyMCE.getParam("relative_urls"))pl.src = tinyMCE.convertRelativeToAbsoluteURL(tinyMCE.settings['base_href'], pl.src);
                    ...
                    if (attribs.width)at = at.replace(/width:[^0-9]?[0-9]+%?[^0-9]?/g, "width:'" + attribs.width + "'");
                    if (attribs.height)at = at.replace(/height:[^0-9]?[0-9]+%?[^0-9]?/g, "height:'" + attribs.height + "'");
    ...
    for (i = 0; i < at.length; i++) {
        if (at[i].specified && at[i].nodeValue) {
            ...
            if (an == 'src')continue;
            if (an == 'mce_src')an = 'src';
            if (an.indexOf('mce_') == -1 && !new RegExp('^(class|type)$').test(an))ti += an.toLowerCase() + ':\'' + at[i].nodeValue + "',"
        }
    }
    ...
    for (an in al) {
        if (al[an] != null && typeof(al[an]) != "function" && al[an] != '')ti += an.toLowerCase() + ':\'' + al[an] + "',"
    }
    ti = ti.length > 0 ? ti.substring(0, ti.length - 1) : ti;
    ne.title = ti;
    return ne
    ...
    for (n in p) {
        if (p[n] && typeof(p[n]) != "function") {
            ...
            if (n == 'src' && p[n].indexOf('://') != -1 && mt == 'application/x-mplayer2')h += '<param name="url" value="' + p[n] + '" />'
        }
    }
    h += '<embed type="' + mt + '"';
    for (n in p) {
        if (typeof(p[n]) == "function")continue;
        if (!(n == 'url' && mt == 'application/x-mplayer2'))h += ' ' + n + '="' + p[n] + '"'
    }
    h += '></embed></object>';
    return h
    ...
    for (var i = 0; i < attribute_string.length; i++) {
        ...
        if ((chr == '"' || chr == "'") && !withInValue)withInValue = true; else if ((chr == '"' || chr == "'") && withInValue) {
            withInValue = false;
            ...
            attributeName = "";
            attributeValue = ""
        } else if (!whiteSpaceRegExp.test(chr) && !withInName && !withInValue)withInName = true;
