...
TinyMCE_Engine.prototype = {init: function (settings) {
    ...
    var op = this.plugins[n];
    if (op)op.baseURL = u; else this.plugins[n] = {baseURL: u}
        ...
        switch (mode) {
            case"exact":
                ...
                for (var i = 0; i < elements.length; i++) {
                    ...
                    if (element)tinyMCE.addMCEControl(element, elements[i]); else if (tinyMCE.settings['debug'])alert("Error: Could not find element by id or name: " + elements[i])
                }
                break;
...
function TinyMCE_Control(settings) {
    ...
    this.data = {};
    this.cleanup.init({valid_elements: s.valid_elements, extended_valid_elements: s.extended_valid_elements, valid_child_elements: s.valid_child_elements, entities: s.entities, entity_encoding: s.entity_encoding, debug: s.cleanup_debug, url_converter: 'TinyMCE_Cleanup.prototype._urlConverter', indent: s.apply_source_formatting, invalid_elements: s.invalid_elements, verify_html: s.verify_html, fix_content_duplication: s.fix_content_duplication});
...
TinyMCE_Control.prototype = {selection: null, settings: null, cleanup: null, getData: function (na) {
    ...
    if (tinyMCE.settings['force_br_newlines']) {
        ...
        if (doc.selection.type != "Control") {
            ...
            if (alignValue != "") {
                ...
                if ((divElm = tinyMCE.getParentElement(rng.parentElement(), "div")) != null)divElm.setAttribute("align", alignValue); else if (rng.pasteHTML && rng.htmlText.length > 0)rng.pasteHTML('<div align="' + alignValue + '">' + rng.htmlText + "</div>");
                tinyMCE.triggerNodeChange();
                return
            }
        }
    ...
    switch (command) {
        ...
        case"SetStyleInfo":
            ...
            if (tinyMCE.isIE) {
                if (rng.item)parentElm = rng.item(0); else {
...
TinyMCE_Cleanup.prototype = {init: function (s) {
    ...
    for (x = 0; x < ta.length; x++) {
        ...
        for (y = 0; y < tn.length; y++) {
            ...
            for (i = 0; i < cl.length; i++) {
                if (cl[i] == '%istrict')r += tinyMCE.inlineStrict; else if (cl[i] == '%itrans')r += tinyMCE.inlineTransitional; else if (cl[i] == '%istrict_na')r += tinyMCE.inlineStrict.substring(2); else if (cl[i] == '%itrans_na')r += tinyMCE.inlineTransitional.substring(2); else if (cl[i] == '%btrans')r += tinyMCE.blockElms; else if (cl[i] == '%strict')r += tinyMCE.blockStrict; else r += (cl[i].charAt(0) != '#' ? cl[i].toUpperCase() : cl[i]);
                r += (i != cl.length - 1 ? '|' : '')
            }
            r += ')$';
    ...
    if (!c.settings.on_save)return tinyMCE.convertRelativeToAbsoluteURL(tinyMCE.settings.base_href, v); else if (tinyMCE.getParam('convert_urls'))return eval(tinyMCE.settings.urlconverter_callback + "(v, n, true);");
    return v
...
TinyMCE_Engine.prototype.setOuterHTML = function (e, h, d) {
    ...
    if (tinyMCE.isIE && e.nodeType == 1)e.outerHTML = h; else {
...
TinyMCE_Selection.prototype = {getSelectedHTML: function () {
    ...
    if (r.cloneContents)e.appendChild(r.cloneContents()); else if (typeof(r.item) != 'undefined' || typeof(r.htmlText) != 'undefined')e.innerHTML = r.item ? r.item(0).outerHTML : r.htmlText; else e.innerHTML = r.toString();
    ...
    sx = vp.left;
    sy = vp.top;
    if (tinyMCE.isSafari || tinyMCE.isOpera || simple)return{rng: rng, scrollX: sx, scrollY: sy};
    if (tinyMCE.isIE) {
        if (rng.item) {
            e = rng.item(0);
