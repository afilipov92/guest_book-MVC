...
var TinyMCE_TinyTheme = {
	...
	getControlHTML : function(button_name) {
		...
		for (i=0; i<TinyMCE_TinyTheme._buttons.length; i++) {
			but = TinyMCE_TinyTheme._buttons[i];
		...
		switch (button_name) {
			...
			case "styleselect":
				return '<select id="{$editor_id}_styleSelect" onmousedown="tinyMCE.themes.tiny._setupCSSClasses(\'{$editor_id}\');" name="{$editor_id}_styleSelect" onfocus="tinyMCE.addSelectAccessibility(event,this,window);" onchange="tinyMCE.execInstanceCommand(\'{$editor_id}\',\'mceSetCSSClass\',false,this.options[this.selectedIndex].value);" class="mceSelectList">{$style_select_options}</select>';
	...
	execCommand : function(editor_id, element, command, user_interface, value) {
		switch (command) {
			...
			case "mceLink":
				...
				if (inst.settings['insertlink_callback']) {
					...
					if (returnVal && returnVal['href'])
						TinyMCE_TinyTheme._insertLink(returnVal['href'], returnVal['target'], returnVal['title'], returnVal['onclick'], returnVal['style_class']);
				} else {
			...
			case "mceImage":
				...
				if (inst.settings['insertimage_callback']) {
					...
					if (returnVal && returnVal['src'])
						TinyMCE_TinyTheme._insertImage(returnVal['src'], returnVal['alt'], returnVal['border'], returnVal['hspace'], returnVal['vspace'], returnVal['width'], returnVal['height'], returnVal['align'], returnVal['title'], returnVal['onmouseover'], returnVal['onmouseout']);
				} else
			...
			case "forecolor":
				...
				TinyMCE_TinyTheme._hideMenus(editor_id);
				...
				if (!fcp.exists()) {
					...
					tinyMCE.addEvent(elm, 'click', TinyMCE_TinyTheme._handleMenuEvent);
					tinyMCE.addEvent(elm, 'mouseover', TinyMCE_TinyTheme._handleMenuEvent);
					tinyMCE.addEvent(elm, 'mouseout', TinyMCE_TinyTheme._handleMenuEvent);
				}
			...
			case "forecolorMenu":
				...
				if (!ml.exists())
					ml.create('div', 'mceMenu', document.body, TinyMCE_TinyTheme._getColorHTML(editor_id, 'theme_advanced_text_colors', 'forecolor'));
			...
			case "HiliteColorMenu":
				...
				if (!ml.exists())
					ml.create('div', 'mceMenu', document.body, TinyMCE_TinyTheme._getColorHTML(editor_id, 'theme_advanced_background_colors', 'HiliteColor'));
	...
	getEditorTemplate : function(settings, editorId) {
		...
		var statusbarHTML = '<div id="{$editor_id}_path" class="mceStatusbarPathText" style="display: ' + (path ? "block" : "none") + '">&#160;</div><div id="{$editor_id}_resize" class="mceStatusbarResize" style="display: ' + (resizing ? "block" : "none") + '" onmousedown="tinyMCE.themes.tiny._setResizing(event,\'{$editor_id}\',true);"></div><br style="clear: both" />';
		...
		if (settings['theme_advanced_styles']) {
			...
			TinyMCE_TinyTheme._autoImportCSSClasses = false;
		}
	...
	initInstance : function(inst) {
		if (tinyMCE.getParam("theme_advanced_resizing", false)) {
			if (tinyMCE.getParam("theme_advanced_resizing_use_cookie", true)) {
				var w = TinyMCE_TinyTheme._getCookie("TinyMCE_" + inst.editorId + "_width");
				var h = TinyMCE_TinyTheme._getCookie("TinyMCE_" + inst.editorId + "_height");

				TinyMCE_TinyTheme._resizeTo(inst, w, h, tinyMCE.getParam("theme_advanced_resize_horizontal", true));
			}
		}
	...
	handleNodeChange : function(editor_id, node, undo_index, undo_levels, visual_aid, any_selection, setup_content) {
		...
		if (selectElm) {
			TinyMCE_TinyTheme._setupCSSClasses(editor_id);
	...
	_setupCSSClasses : function(editor_id) {
		...
		if (!TinyMCE_TinyTheme._autoImportCSSClasses)
			return;
	...
	_resizeEventHandler : function(e) {
		var resizer = TinyMCE_TinyTheme._resizer;
		...
		switch (e.type) {
			...
			case "mouseup":
				TinyMCE_TinyTheme._setResizing(e, editorId, false);
				TinyMCE_TinyTheme._resizeTo(tinyMCE.getInstanceById(editorId), resizer.width + dx, resizer.height + dy, resizer.horizontal);
				...
				if (tinyMCE.getParam("theme_advanced_resizing_use_cookie", true)) {
					...
					// Set the cookies
					TinyMCE_TinyTheme._setCookie("TinyMCE_" + editorId + "_width", "" + (resizer.horizontal ? resizer.width + dx : ""), expires);
					TinyMCE_TinyTheme._setCookie("TinyMCE_" + editorId + "_height", "" + (resizer.height + dy), expires);
				}
				break;
		}
	},
	...
	_setResizing : function(e, editor_id, state) {
		...
		if (state) {
			...
			if (!resizer.eventHandlers) {
				if (tinyMCE.isMSIE)
					tinyMCE.addEvent(document, "mousemove", TinyMCE_TinyTheme._resizeEventHandler);
				else
					tinyMCE.addEvent(window, "mousemove", TinyMCE_TinyTheme._resizeEventHandler);

				tinyMCE.addEvent(document, "mouseup", TinyMCE_TinyTheme._resizeEventHandler);
	...
	_getColorHTML : function(id, n, cm) {
		...
		cl = tinyMCE.getParam(n, TinyMCE_TinyTheme._defColors).split(',');
...
tinyMCE.addTheme("tiny", TinyMCE_TinyTheme);

// Add default buttons maps for advanced theme and all internal plugins
tinyMCE.addButtonMap(TinyMCE_TinyTheme._buttonMap);
