...
var TinyMCE_DevKitPlugin = {_logFilter: '\\[(importCSS|execCommand|execInstanceCommand|debug)\\]', _logPadding: '', _startTime: null, _benchMark: false, _winLoaded: false, _isDebugEvents: false, getInfo: function () {
    ...
    if (this._logPadding.length > 0)this._logPadding = this._logPadding.substring(0, this._logPadding.length - 1);
    if (this._benchMark)this._log("benchmark", "Execution time: " + (new Date().getTime() - st))
}, _log: function (t) {
    ...
    if (!this._winLoaded)tinyMCE.log[tinyMCE.log.length] = m; else e.contentWindow.debug(m)
    ...
    if (TinyMCE_DevKitPlugin._isDebugEvents == s)return;
    TinyMCE_DevKitPlugin._isDebugEvents = s;
    for (n in tinyMCE.instances) {
        inst = tinyMCE.instances[n];
        if (!tinyMCE.isInstance(inst) || inst.getDoc() == ld)continue;
        ld = inst.getDoc();
        for (i = 0; i < ev.length; i++) {
            if (s)tinyMCE.addEvent(ld, ev[i], TinyMCE_DevKitPlugin._debugEvent); else tinyMCE.removeEvent(ld, ev[i], TinyMCE_DevKitPlugin._debugEvent)
        }
    }
...
tinyMCE.debug = function () {
    ...
    if (!start)TinyMCE_DevKitPlugin._startTime = start = now;
