...
var TinyMCE_TablePlugin = {getInfo: function () {
    ...
    for (var i = 0; i < controls.length; i++) {
        ...
        if (but[0] == control_name)return tinyMCE.getButtonHTML(control_name, but[2], '{$pluginurl}/images/' + but[1], but[3], (but.length > 4 ? but[4] : false))
    }
    ...
    if (tdElm = tinyMCE.getParentElement(node, "td,th")) {
        ...
        if (colspan != "1" || rowspan != "1")tinyMCE.switchClass(editor_id + '_split_cells', 'mceButtonNormal')
    }
    if (tinyMCE.getParentElement(node, "tr"))tinyMCE.switchClass(editor_id + '_row_props', 'mceButtonSelected');
    ...
    function addRows(td_elm, tr_elm, rowspan) {
        ...
        for (var i = 1; i < rowspan && trNext; i++) {
            var newTD = doc.createElement("td");
            newTD.innerHTML = "&nbsp;";
            if (tinyMCE.isMSIE)trNext.insertBefore(newTD, trNext.cells(td_elm.cellIndex)); else trNext.insertBefore(newTD, trNext.cells[td_elm.cellIndex]);
            ...
            if (tableElm && trElm) {
                switch (command) {
                    ...
                    case"mceTableDeleteCol":
                        ...
                        cpos.cellindex--;
                        if (cpos.cellindex < 0)cpos.cellindex = 0;
                        grid = getTableGrid(tableElm);
                        inst.selection.selectNode(getCell(grid, cpos.rowindex, 0), tinyMCE.isGecko, true);
                        break;
                    case"mceTableSplitCells":
                    ...
                    case"mceTableMergeCells":
                        ...
                        if (tinyMCE.isMSIE || sel.rangeCount == 1) {
                            ...
                            for (var i = 0; i < sel.rangeCount; i++) {
                                ...
                                if (!tdElm)break;
                                if (tdElm.nodeName == "TD")cells[cells.length] = tdElm
                            }
                            var tRows = tableElm.rows;
                        ...
                        for (var y = 0; y < rows.length; y++) {
                            for (var x = 0; x < rows[y].length; x++) {
                                ...
                                if (chk != "<br/>" && chk != "<br>" && chk != "&nbsp;" && (x + y > 0))tdElm.innerHTML += html;
