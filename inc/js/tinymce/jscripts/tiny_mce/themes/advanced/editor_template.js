...
var TinyMCE_AdvancedTheme = {_defColors: "000000,993300,333300,003300,003366,000080,333399,333333,800000,FF6600,808000,008000,008080,0000FF,666699,808080,FF0000,FF9900,99CC00,339966,33CCCC,3366FF,800080,999999,FF00FF,FFCC00,FFFF00,00FF00,00FFFF,00CCFF,993366,C0C0C0,FF99CC,FFCC99,FFFF99,CCFFCC,CCFFFF,99CCFF,CC99FF,FFFFFF", _autoImportCSSClasses: true, _resizer: {}, _buttons: [
    ...
    for (i = 0; i < TinyMCE_AdvancedTheme._buttons.length; i++) {
        ...
        if (but[0] == button_name && (button_name == "forecolor" || button_name == "backcolor"))return tinyMCE.getMenuButtonHTML(but[0], but[2], '{$themeurl}/images/' + but[1], but[3] + "Menu", but[3], (but.length > 4 ? but[4] : false), (but.length > 5 ? but[5] : null));
        if (but[0] == button_name)return tinyMCE.getButtonHTML(but[0], but[2], '{$themeurl}/images/' + but[1], but[3], (but.length > 4 ? but[4] : false), (but.length > 5 ? but[5] : null))
    }
    switch (button_name) {
        case"formatselect":
            ...
            for (var i = 0; i < formats.length; i++) {
                for (var x = 0; x < lookup.length; x++) {
                    if (formats[i] == lookup[x][0])html += '<option value="&lt;' + lookup[x][0] + '&gt;">' + lookup[x][1] + '</option>'
                }
            }
            html += '</select>';
            return html;
    ...
    switch (command) {
        ...
        case"mceLink":
            ...
            if (!tinyMCE.linkElement) {
                if ((tinyMCE.selectedElement.nodeName.toLowerCase() != "img") && (selectedText.length <= 0))return true
            }
            ...
            if (inst.settings['insertlink_callback']) {
                ...
                if (returnVal && returnVal['href'])TinyMCE_AdvancedTheme._insertLink(returnVal['href'], returnVal['target'], returnVal['title'], returnVal['onclick'], returnVal['style_class'])
            } else {
        ...
        case"mceImage":
            ...
            if (img) {
                if (tinyMCE.getAttrib(img, 'name').indexOf('mce_') == 0)return true;
            ...
            if (inst.settings['insertimage_callback']) {
                ...
                if (returnVal && returnVal['src'])TinyMCE_AdvancedTheme._insertImage(returnVal['src'], returnVal['alt'], returnVal['border'], returnVal['hspace'], returnVal['vspace'], returnVal['width'], returnVal['height'], returnVal['align'], returnVal['title'], returnVal['onmouseover'], returnVal['onmouseout'])
            } else tinyMCE.openWindow(template, {src: src, alt: alt, border: border, hspace: hspace, vspace: vspace, width: width, height: height, align: align, title: title, onmouseover: onmouseover, onmouseout: onmouseout, action: action, inline: "yes"});
            return true;
        case"forecolor":
        ...
        case"forecolorMenu":
            ...
            if (!ml.exists())ml.create('div', 'mceMenu', document.body, TinyMCE_AdvancedTheme._getColorHTML(editor_id, 'theme_advanced_text_colors', 'forecolor'));
            ...
            if (tinyMCE.isOpera)ml.moveBy(0, -2);
            ml.show();
            return true;
        case"HiliteColor":
        ...
        case"HiliteColorMenu":
            ...
            if (!ml.exists())ml.create('div', 'mceMenu', document.body, TinyMCE_AdvancedTheme._getColorHTML(editor_id, 'theme_advanced_background_colors', 'HiliteColor'));
        ...
        case"mceColorPicker":
            if (user_interface) {
                ...
                if (typeof(value['store_selection']) == "undefined")value['store_selection'] = true;
                ...
                elm.value = value;
                if (elm.onchange != null && elm.onchange != '')eval('elm.onchange();')
            }
            return true;
        case"mceCodeEditor":
            var template = new Array();
        ...
        case"mceInsertAnchor":
            ...
            return true;
        case"mceNewDocument":
            if (confirm(tinyMCE.getLang('lang_newdocument')))tinyMCE.execInstanceCommand(editor_id, 'mceSetContent', false, ' ');
            return true
    }
    return false
    ...
    switch (layoutManager) {
        case"SimpleLayout":
            ...
            if (!tinyMCE.getParam("accessibility_focus"))toolbarHTML += ' onfocus="tinyMCE.getInstanceById(\'' + editorId + '\').getWin().focus();"';
            toolbarHTML += '></a>';
            for (var i = 1; i < 100; i++) {
                ...
                for (var b = 0; b < buttons.length; b++)toolbarHTML += tinyMCE.getControlHTML(buttons[b]);
                if (buttons.length > 0) {
                    toolbarHTML += "<br />";
                    deltaHeight -= 23
                }
        ...
        case"RowLayout":
            ...
            for (var i = 0; i < containers.length; i++) {
                if (containers[i] == "mceEditor")template['html'] += '<tr><td align="center" class="mceEditor_border"><span id="{$editor_id}"></span></td></tr>'; else if (containers[i] == "mceElementpath" || containers[i] == "mceStatusbar") {
                    ...
                    for (var j = 0; j < curContainer.length; j++)curContainerHTML += tinyMCE.getControlHTML(curContainer[j]);
    ...
    if (resizing)template['html'] += '<span id="{$editor_id}_resize_box" class="mceResizeBox"></span>';
    ...
    if (e.type == "click")tinyMCE.execInstanceCommand(te._editor_id, te._command)
    ...
    if (pathElm) {
        ...
        for (var i = path.length - 1; i >= 0; i--) {
            ...
            var nodeData = "";
            if (nodeName.indexOf("html:") == 0)nodeName = nodeName.substring(5);
            if (nodeName == "b") {
                nodeName = "strong"
            }
            if (nodeName == "i") {
            ...
            if (nodeName == "span") {
                ...
                if (cn != "" && cn.indexOf('mceItem') == -1)nodeData += "class: " + cn + " ";
            ...
            if (nodeName == "font") {
                if (tinyMCE.getParam("convert_fonts_to_spans"))nodeName = "span";
                var face = tinyMCE.getAttrib(path[i], "face");
                if (face != "")nodeData += "font: " + face + " ";
                var size = tinyMCE.getAttrib(path[i], "size");
                if (size != "")nodeData += "size: " + size + " ";
                var color = tinyMCE.getAttrib(path[i], "color");
                if (color != "")nodeData += "color: " + color + " "
            }
            if (getAttrib(path[i], 'id') != "") {
                nodeData += "id: " + path[i].getAttribute('id') + " "
            }
            var className = tinyMCE.getVisualAidClass(tinyMCE.getAttrib(path[i], "class"), false);
            if (className != "" && className.indexOf('mceItem') == -1)nodeData += "class: " + className + " ";
            if (getAttrib(path[i], 'src') != "") {
                var src = tinyMCE.getAttrib(path[i], "mce_src");
                if (src == "")src = tinyMCE.getAttrib(path[i], "src");
                nodeData += "src: " + src + " "
            }
            if (path[i].nodeName == 'A' && getAttrib(path[i], 'href') != "") {
                var href = tinyMCE.getAttrib(path[i], "mce_href");
                if (href == "")href = tinyMCE.getAttrib(path[i], "href");
                nodeData += "href: " + href + " "
            }
    ...
    if (node.nodeName == "A" && tinyMCE.getAttrib(node, "class").indexOf('mceItemAnchor') != -1)tinyMCE.switchClass(editor_id + '_anchor', 'mceButtonSelected');
    ...
    if (tinyMCE.getParentElement(node, "li,blockquote"))tinyMCE.switchClass(editor_id + '_outdent', 'mceButtonNormal');
    if (undo_index != -1 && (undo_index < undo_levels - 1 && undo_levels > 0))tinyMCE.switchClass(editor_id + '_redo', 'mceButtonNormal');
    if (undo_index != -1 && (undo_index > 0 && undo_levels > 0))tinyMCE.switchClass(editor_id + '_undo', 'mceButtonNormal');
    ...
    if (selectElm) {
        ...
        if (elm)selectByValue(selectElm, "<" + elm.nodeName.toLowerCase() + ">"); else selectByValue(selectElm, "")
    }
    ...
    if (selectElm) {
        if (!tinyMCE.isSafari && !(tinyMCE.isMSIE && !tinyMCE.isOpera)) {
            ...
            if (elm) {
                ...
                if (family == '')family = '' + elm.style.fontFamily;
                if (!selectByValue(selectElm, family, family != ""))selectByValue(selectElm, "")
            } else selectByValue(selectElm, "")
        }
    }
    ...
    if (selectElm) {
        if (!tinyMCE.isSafari && !tinyMCE.isOpera) {
            ...
            if (elm) {
                ...
                if (!selectByValue(selectElm, size))selectByValue(selectElm, "")
            } else selectByValue(selectElm, "0")
        }
    }
    alignNode = node;
    breakOut = false;
    do {
        if (!alignNode.getAttribute || !alignNode.getAttribute('align'))continue;
    ...
    if (div && div.style.textAlign == "center")tinyMCE.switchClass(editor_id + '_justifycenter', 'mceButtonSelected');
    if (!setup_content) {
        var ar = new Array("Bold", "_bold", "Italic", "_italic", "Strikethrough", "_strikethrough", "superscript", "_sup", "subscript", "_sub");
        for (var i = 0; i < ar.length; i += 2) {
            if (inst.queryCommandState(ar[i]))tinyMCE.switchClass(editor_id + ar[i + 1], 'mceButtonSelected')
        }
        if (inst.queryCommandState("Underline") && (node.parentNode == null || node.parentNode.nodeName != "A"))tinyMCE.switchClass(editor_id + '_underline', 'mceButtonSelected')
    }
    do {
    ...
    var i, selectElm;
    if (!TinyMCE_AdvancedTheme._autoImportCSSClasses)return;
    selectElm = document.getElementById(editor_id + '_styleSelect');
    if (selectElm && selectElm.getAttribute('cssImported') != 'true') {
        var csses = tinyMCE.getCSSClasses(editor_id);
        if (csses && selectElm) {
            for (i = 0; i < csses.length; i++)selectElm.options[selectElm.options.length] = new Option(csses[i], csses[i])
        }
        if (csses != null && csses.length > 0)selectElm.setAttribute('cssImported', 'true')
    }
    ...
    w = w < 1 ? 30 : w;
    h = h < 1 ? 30 : h;
    if (set_w)tableElm.style.width = w + "px";
    ...
    if (set_w)iframe.style.width = iw + "px";
    ...
    switch (e.type) {
        case"mousemove":
            ...
            w = w < 1 ? 1 : w;
            h = h < 1 ? 1 : h;
            if (resizer.horizontal)resizeBox.style.width = w + "px";
    ...
    if (state) {
        ...
        if (!resizer.eventHandlers) {
            if (tinyMCE.isMSIE)tinyMCE.addEvent(document, "mousemove", TinyMCE_AdvancedTheme._resizeEventHandler); else tinyMCE.addEvent(window, "mousemove", TinyMCE_AdvancedTheme._resizeEventHandler);
    ...
    if (!tinyMCE.imgElement && tinyMCE.isSafari) {
        ...
        if (!tinyMCE.imgElement && tinyMCE.selectedInstance) {
            if (tinyMCE.isSafari)tinyMCE.execCommand("mceInsertContent", false, '<img src="' + tinyMCE.uniqueURL + '" />'); else tinyMCE.selectedInstance.contentDocument.execCommand("insertimage", false, tinyMCE.uniqueURL);
